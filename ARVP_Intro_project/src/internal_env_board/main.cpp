...
/*Is the above a function of UAVCAN????*/
...
/*this will need to be change for FLEXCAN*/
...
/*This can more than likely stay. My guess is that this runs a time interupt that adds a new value to the running average*/



void setup() {
    Wire.begin();
/*^^^for I2C set up. for Temp/humidty sensor*/
    
    initLeds();
/*^^^ a part of the UAVCAN library, needs replacing (led stuff)*/
...
void loop() {
    ...
    if(timer.check() == 1) {
      ...
      if (timerCounter == SAMPLES_PER_SECOND) {
        ...
        /*^^^^^Used for LCD, which we may or may not use*/
